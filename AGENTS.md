...
- `build_with_dpkg_buildpackage` — run `mk-build-deps -i` + `dpkg-buildpackage -us -uc` (full source + binary build) and copy `.deb`, `.dsc`, `.tar.*`, `.changes`, and `.buildinfo` into `<repo>/dist`. For `3.0 (quilt)` ports it first synthesizes `../<package>_<upstream>.orig.tar.xz` from `git archive HEAD:safe` (minus `debian/`) and wipes `debian/patches/`, since the safe `safe/` tree is the upstream and ports do not maintain a round-trippable patch series.
