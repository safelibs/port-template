...
- `build_with_dpkg_buildpackage` — run `mk-build-deps -i` + `dpkg-buildpackage -us -uc -b` (binary-only) and copy `../*.deb` into `<repo>/dist`. Source-package generation isn't supported by the helper because SafeLibs ports' shape (post-patch `safe/` tree, no upstream tarball) doesn't fit Debian's `3.0 (quilt)` round-trip model.
